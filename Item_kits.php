...
// ✅ PLACE YOUR NEW METHODS HERE BEFORE THIS FINAL BRACE
        public function import()
        {
            $this->load->view('item_kits/import');
        }
        ...
        public function process_import()
{
    if ($_FILES['file']['error'] === UPLOAD_ERR_OK)
    {
        ...
        foreach ($csv as $row)
        {
            ...
            $data = [
                'item_kit_number'   => trim($row[0]),
                'name'              => trim($row[1]),
                'kit_discount'      => trim($row[2]),
                'kit_discount_type' => 1
            ];

            // Save the kit and get its ID
            $this->Item_kit->save($data, -1);
            $kit_id = $this->db->insert_id();

            if (!$kit_id) continue;

            // Build array of items just like the form does
            $item_kit_items = [];
            $sequence = 0;
            ...
            // Save all items to the kit
            $this->Item_kit_items->save($item_kit_items, $kit_id);
        }

        $this->session->set_flashdata('success', 'Item Kits Imported Successfully');
    }
    else
    {
