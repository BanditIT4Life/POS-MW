...
<?php echo form_open($controller_name . '/save/' . $person_info->person_id, array('id'=>'employee_form', 'class'=>'form-horizontal')); ?>
	...
	<div class="tab-content">
        ...
        <?php
// Check if DB already provides the sub-permission so we don't render it twice
$has_delivery_perm = FALSE;
foreach ($all_subpermissions as $p) {
	if ($p->permission_id === 'sales_delivery_report') {
		$has_delivery_perm = TRUE;
		break;
	}
}
?>
					...
					?>
						<li>	
							...
							?>
                  <?php if ($module->module_id === 'sales' && !$has_delivery_perm): ?>
	<ul>
		<li>
			<?php
				// Name follows grant_{permission_id}; value is the permission id
				echo form_checkbox(
					"grant_sales_delivery_report",                           // POST name
					"sales_delivery_report",                                 // value saved
					$this->Employee->has_grant('sales_delivery_report', $person_info->person_id) // checked?
				);
				// Keep menu_group alignment with other sub-perms
				echo form_hidden("menu_group_sales_delivery_report", "--");

				// Label: use lang if present, fallback to readable text
				$label = $this->lang->line('sales_delivery_report');
			?>
			<span class="medium">
				<?php echo ($this->lang->line_tbd('sales_delivery_report') == $label) ? 'Delivery Report' : $label; ?>
			</span>
		</li>
	</ul>
<?php endif; ?>
